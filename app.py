...
@app.route('/profile')
def profile():
    return render_template('profile.html', name=usuario.name)
...
# DAQUI PRA CIMA É FRONTEND
########################################################################################################
# DAQUI PRA BAIXO É O BACKEND
...
def guarda_noticias_no_bd(dataset_top_news, dataset_recomendations):
    ...
    for i in range(len(tupla_principal)):
        ...
        data_publicacao = tupla_principal[i][1]
        ...
        for j in range(i+(i*2), (i*2)+i+3):
            ...
            tabelas['noticia'].insert(titulo = tupla_relacionada[j][0], data_publicacao = tupla_relacionada[j][1], categoria = tupla_relacionada[j][5], texto = tupla_relacionada[j][2], fonte = tupla_relacionada[j][6], link = tupla_relacionada[j][3], imagem = tupla_relacionada[j][4])
...
sched.add_job(atualiza_bd,'interval',seconds=90)
sched.start()
