...
from agregador_de_noticias_recommendation_system.src.sentence_cleaner import SentenceCleaner
from agregador_de_noticias_recommendation_system.src.data_scorer_calculator import DataScorerCalculator
from agregador_de_noticias_recommendation_system.src.sentences_similarity_calculator import SentenceSimilarityCalculator
...
class RunRecomendationSystem:
    ...
    def run(self, dataset_topnews, dataset_to_recommendation):
        ...
        self.new_doc = dataset_to_recommendation.copy()
        ...
        self.dataset_topnews.loc[:, 'sentences'] = dataset_topnews.loc[:, 'title'].apply(self.sentece_cleaner.clear_sentence)
        #new_sent = self.sentece_cleaner.clear_sentence(raw_sentence)
        #new_sent = self.tfidf_vectorizer.transform([new_sent])

        dataset_to_recommendation_tfidf_matrix = self.tfidf_vectorizer.transform(self.new_doc['sentences'].values)
        ...
        for new_sentence in self.dataset_topnews['sentences'].values:
            ...
            new_sent = self.tfidf_vectorizer.transform([new_sent])
            self.idx_recomendations.extend(self.sentences_sym_calculator.sym_by_date(new_sent, dataset_to_recommendation_tfidf_matrix, self.new_doc['normalized_data_scores'].values, 20, 3))
